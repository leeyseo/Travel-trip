...
def _geocode(place_name: str, destination: str) -> tuple | None:
    """Nominatim(OpenStreetMap) 무료 geocoding — 여러 쿼리 시도"""
    try:
        from geopy.geocoders import Nominatim
        geolocator = Nominatim(user_agent="travel_agent_v2")
        # 한국어 → 영어 간단 매핑 (자주 쓰이는 장소)
        ko_en = {
            "북촌 한옥마을": "Bukchon Hanok Village",
            "남한산성 둘레길": "Namhansanseong",
            "정자동 카페거리": "Jeongja-dong Cafe Street Seongnam",
            "감천 문화마을": "Gamcheon Culture Village Busan",
            "누리마루 APEC하우스": "Nurimaru APEC House Busan",
            "신세계사이먼 부산": "Shinsegae Simon Busan",
            "클럽디 오아시스": "Club D Oasis Busan",
        }
        en_name = ko_en.get(place_name, "")

        # "본점", "1호점" 등 제거한 단순화 이름도 시도
        ...
        if simplified != place_name:
            queries.insert(0, f"{simplified}, {destination}, 대한민국")
            queries.append(f"{simplified} {destination}")
        if en_name:
            queries.insert(0, en_name)
        else:
            queries += [f"{place_name} {destination}", place_name]
        if simplified != place_name:
            queries.append(simplified)
        # 식당의 경우 "restaurant" 키워드 추가
        queries.append(f"{simplified} restaurant {destination}")
        for query in queries:
            time.sleep(1.1)
            location = geolocator.geocode(query, timeout=10)
            if location:
                print(f"  [Geocode OK] ({location.latitude:.4f}, {location.longitude:.4f})")
                return (location.latitude, location.longitude)
    except Exception as e:
        print(f"  [Geocode WARN] {place_name}: {e}")
    return None
